...
novtheme.Countdown = function () {
  function startCountdown($countdown, showDays, restartCountdown) {
    ...
    var dayString = showDays
      ? '<div class="item-time"><div class="data-number">%D</div><div class="name-time">Days</div></div>'
      : "";
...
novtheme.AddActive = function () {
  ...
  $("[nov-btn-close]").click(function () {
    ...
    $('[data-act="' + data + '"], .sidebar-overlay, [nov-btn-act]').removeClass(
      "act"
    );
...
novtheme.PageSizeGuild = function () {
  ...
  $(".size-guides__links-division").on("click", "a", function (e) {
    ...
    if ($("#nov-sizeguide-product").length > 0) {
      $("#nov-sizeguide-product").animate(
        {
          scrollTop: $(
            '.size-guides__container[data-scrolled="' + link + '"]'
          ).position().top,
        },
        "5000",
        "swing"
